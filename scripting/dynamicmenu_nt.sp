...
BuildDynamicMenu()
{
	...
	{
		BuildPath(Path_SM, file, 255, "configs/nt_menu.txt");
	}
	...
	{		
		...
		if ((categoryId = hTopMenu.FindCategory(buffer)) == INVALID_TOPMENUOBJECT)
		{
			categoryId = hTopMenu.AddCategory(buffer,
...
ParseConfigs()
{
	...
	{
		BuildPath(Path_SM, configPath, sizeof(configPath), "configs/nt_menu_grouping.txt");
	}
	
	if (!FileExists(configPath))
	{
		LogError("Unable to locate menu groups file: %s", configPath);
