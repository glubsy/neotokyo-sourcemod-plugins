...
public OnPluginStart()
{
	...
	g_hForwardVotedAllTalk = CreateGlobalForward("OnVotedAllTalk", ET_Event, Param_Cell); //Neotokyo

	g_Cvar_Gravity = FindConVar("sv_gravity");
	//g_Cvar_Alltalk = FindConVar("sv_alltalk");  //modified -glub
...
{
	g_Cvar_Alltalk = FindConVar("sv_alltalk"); //change to nt_alltalk if using voicefix in nt_selfmute -glub
}
		...
		else
		{
			...
			switch (g_voteType)
			{
				...
				case (voteType:alltalk):
				{
					...
					PushOnVotedAllTalk(GetConVarInt(g_Cvar_Alltalk)); //Neotokyo
				}
...
void PushOnVotedAllTalk(int value) //Neotokyo
{
	...
	#if DEBUG > 0
	PrintToServer("funvotes: voted sv_alltalk %b", value);
	#endif

	Call_StartForward(g_hForwardVotedAllTalk);
	Call_PushCell(value);
	Call_Finish();
}